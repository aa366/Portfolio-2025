...
const Project = ({ name, image, category, techstack, links }: project) => {
    ...
    return (
        <motion.div
            ...
            <div className="relative group rounded-lg bg-violet-50" onClick={()=>setActive(!active)}>
                ...
                {(links.visit.trim() || links.code.trim() || links.video.trim()) &&
                    <div className={`absolute top-0 scale-x-0 ${active && "scale-100"} group-hover:scale-100 ${active && "scale-x-100"} transition-transform origin-left duration-200 ease-linear bg-gray-800 bg-opacity-60 w-full h-full rounded-lg flex items-center gap-4 justify-center`} onClick={()=>setActive(!active) }>
                        {links.visit.trim() &&
